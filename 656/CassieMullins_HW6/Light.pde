public class Light {
  ...
  public Vector calculateColor(Hit hit) {
    return calculateColorFromLight(new Vector(0,0,0), Cl, hit, false);
    }
}
...
class Point extends Light {
  ...
  public Point(Vector pos, Vector col) {
   ...
   public Vector calculateColor(Hit hit) {
    ...
    return calculateColorFromLight(L, Cl, hit, hasShadow);
    }
...
class Direction extends Light {
  ...
  public Vector calculateColor(Hit hit) {
    ...
    return calculateColorFromLight(dir, Cl, hit, hasShadow);
  }
...
class RectangleArea extends Light {
   ...
   public Vector calculateColor(Hit hit) {
    ...
    for (Point light : pointLights) {
      Vector pointCol = light.calculateColor(hit);
